...
def test_tags_show():
    with temp_audio_file() as temp_audio:
        runner = CliRunner()
        result = runner.invoke(cli, ['--json', 'tags-show', temp_audio])
        assert loads(result.output) == {}

def test_tags_update():
    with temp_audio_file() as temp_audio:
        runner = CliRunner()
        result = runner.invoke(cli, ['--json', 'tags-update', temp_audio, 'artist=foo'])
...
def test_picture_update_and_extract():
    with temp_audio_file() as temp_audio:
        with temp_image_file(suffix='.jpg') as temp_image:
            runner = CliRunner()
            result = runner.invoke(cli, ['--json', 'picture-update', temp_audio, temp_image])
            assert result.output == 'true\n'

            with NamedTemporaryFile(suffix='.jpg') as new_file:
                result = runner.invoke(cli, ['--json', 'picture-extract', temp_audio, new_file.name])
